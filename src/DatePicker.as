package 
{
	...
	public class DatePicker extends Sprite 
	{
		...
		public var _selectDateIndex:int = 0;
		...
		private var _curMonthIndex:int;
...
//			dayRow = this.addChild(drawDayNameRow(5, 25)) as Sprite;
		...
		private function onNextClick(e : MouseEvent) : void
		{
			...
			// make DateGrid-Sprite for the next month
		...
		private function drawDateGrid(mm : Number, yy : Number) : void
		{
			...
			monthTf.text = "第" + (_curMonthIndex) + "个月";
			monthTf.setTextFormat(_tfmMonth);
			...
			{	
				...
				isCurSelected = i == _selectDateIndex;
				
				//** 每个日期数字
				var gridItem : Sprite = new Sprite();
				gridItem.name = i + "." + (mm) + "." + yy;
				gridItem.name = i.toString();
				gridItem.x = dayNameNr * 32;
				gridItem.y = row * 20 + 20;
			...
			_disallowPrev = _curMonthIndex <= 0;
		...
		private function onDateClick(e : MouseEvent) : void
		{
			...
			this._selectDateIndex = int((e.target as Sprite).name);
		...
		public function get curMonthIndex():int
		{
			return _curMonthIndex;
		}

		public function set curMonthIndex(value:int):void
		{
			_curMonthIndex = value;
			clearDateGrid();
			this.drawDateGrid(0,0);
		}
